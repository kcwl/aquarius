...
namespace virgo
{
	std::expected<std::string, std::string> protocol::parse(std::fstream& ifs, std::size_t& column, std::size_t& row)
	{
		return read_value<'{'>(ifs, column, row)
			.and_then(
				[&](const auto& value) -> std::expected<std::string, std::string>
				{
					...
					{
						auto result =
							read_value<'{'>(ifs, column, row, token::key)
								.and_then(
									[&](const auto& value) -> std::expected<std::string, std::string>
									{
										...
										if (value == "request")
											result = request_.parse(name_ + "_req", ifs, column, row);
										else if (value == "response")
											result = response_.parse(name_ + "_resp", ifs, column, row);
										else if (value == "router")
											result = router_.parse(ifs, column, row);
										else
											return std::unexpected("syntax error! Currently only support keywords that "
																   "is router, request and response!");
					...
					if (!res.has_value())
						return std::unexpected("miss end ['}']!");
	...
	std::expected<bool, std::string> structure::parse(std::fstream& ifs, std::size_t column, std::size_t row)
	{
		return read_value<'{'>(ifs, column, row)
			.and_then(
				[&](const auto& value) -> std::expected<bool, std::string>
				{
					name_ = value;
	...
	std::expected<std::string, std::string> enum_struct::parse(std::fstream& ifs, std::size_t column, std::size_t row)
	{
		return read_value<'{'>(ifs, column, row)
			.and_then(
				[&](const auto& value) -> std::expected<std::string, std::string>
