...
namespace aquarius
{
	namespace gateway
	{
		...
		{
			srvd_host_ = srv_config::get_mutable_instance().host;
			srvd_port_ = srv_config::get_mutable_instance().port;
		...
		{
			co_await mpc_async_call<&serviced::srvd_client::publish>(group_, srvd_host_, srvd_port_);
