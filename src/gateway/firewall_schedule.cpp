...
namespace aquarius
{
	namespace gateway
	{
		...
		{
			co_return co_await mpc::call<bool, firewall_module>(
				"firewall_module", [&](firewall_module* ptr) -> awaitable<bool> { co_return ptr->check_ip(ip); });
		}

		auto mpu_firewall_check_mac(const std::string& mac) -> awaitable<bool>
		{
			co_return co_await mpc::call<bool, firewall_module>(
				"firewall_module", [&](firewall_module* ptr) -> awaitable<bool> { co_return ptr->check_mac(mac); });
		}
	} // namespace gateway
} // namespace aquarius