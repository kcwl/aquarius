...
namespace aquarius
{
	namespace ip
	{
		...
		inline auto mpu_transfer(std::shared_ptr<Request> request, error_code& ec) -> awaitable<flex_buffer>
		{
			co_return co_await mpc::call<flex_buffer, T>([&](T* ptr) -> awaitable<flex_buffer>
														 {
