...
namespace aquarius
{
	namespace serviced
	{
		auto mpc_publish(std::string_view topic, flex_buffer& buffer, std::size_t id, error_code& ec) -> awaitable<flex_buffer>
		{
			co_return co_await mpc::call<flex_buffer, channel_module>(
				"channel_module",
				[&](channel_module* ptr) -> awaitable<flex_buffer> { co_return co_await ptr->publish(topic, buffer, id, ec); });
		}
