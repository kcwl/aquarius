...
namespace aquarius
{
	...
	inline auto mpc_player_insert(std::string_view module_name, std::size_t id, std::shared_ptr<T> player)
		-> awaitable<void>
	{
		co_return co_await mpc::call<void, player_module<T>>(
			module_name, [&](player_module<T>* ptr) -> awaitable<void> { co_return ptr->insert(id, player); });
	}

	template <typename T>
	inline auto mpc_player_erase(std::string_view module_name, std::size_t id) -> awaitable<void>
	{
		co_return co_await mpc::call<void, player_module<T>>(module_name, [&](player_module<T>* ptr) -> awaitable<void>
	...
	inline auto mpc_player_get(std::string_view module_name, std::size_t id) -> awaitable<std::shared_ptr<T>>
	{
		co_return co_await mpc::call<std::shared_ptr<T>, player_module<T>>(
			module_name,
			[&](player_module<T>* ptr) -> awaitable<std::shared_ptr<T>> { co_return ptr->get(id); });
	}
} // namespace aquarius