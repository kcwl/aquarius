...
namespace aquarius
{
	...
	class basic_connect : public std::enable_shared_from_this<basic_connect<_Protocol, _ServerType, _Crypto>>
	{
		constexpr static bool is_server = _ServerType::value;

		using this_type = basic_connect<_Protocol, _ServerType, _Crypto>;

		using crypto_t = _Crypto<_ServerType>;

	public:
		explicit basic_connect(asio::socket_t socket, asio::ssl_context_t& ctx)
		...
		basic_connect(asio::io_service& io_service, asio::ssl_context_t& basic_context)
			: basic_connect(std::move(asio::ip::tcp::socket(io_service)), basic_context)
		{}
