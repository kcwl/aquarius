...
namespace aquarius
{
	...
	class basic_session<false, Protocol> : public std::enable_shared_from_this<basic_session<false, Protocol>>
	{
	public:
#ifdef AQUARIUS_ENABLE_SSL
		using socket_type = typename detail::ssl_session_service<false, Protocol>::ssl_socket_type;
		using impl_type = detail::session_object_impl<detail::ssl_session_service<false, Protocol>>;
#else
		using socket_type = typename detail::session_service<Protocol>::socket_type;
		using impl_type = detail::session_object_impl<detail::session_service<Protocol>>;
#endif
	...
	public:
		...
		template<typename BufferSequence>
		auto async_write_some(BufferSequence buffer, error_code& ec) -> awaitable<error_code>
		{
			co_await impl_.get_service().async_write_some(impl_.get_implementation(), std::move(buffer), ec);
