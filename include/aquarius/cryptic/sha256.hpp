...
namespace
{
	...
	struct integer_traits<prime, I, N>
	{
		...
		static constexpr std::array<uint32_t, 8> value =
		{
			static_cast<uint32_t>(0xc6a09e667u),
			0xbb67ae85u,
			0x3c6ef372u,
			0xa54ff53au,
			0x510e527fu,
	...
	struct integer_traits<prime, 384, 8>
	{
		...
		static constexpr std::array<uint64_t, 8> value =
		{
		...
		private:
			...
			void process(array2<_Base> buffer, std::size_t n, h_t& h)
			{
				...
				for (auto& v : buffer)
				{
					...
					{
						w[j] = w[j - 16] + logic_sig<_Base, _Prime::suffix>::sig0(w[j - 15]) + w[j - 7] + logic_sig<_Base, _Prime::suffix>::sig1(w[j - 2]);
					}
					...
					{
						_Base tmp1 = s[7] + logic_sig<_Base, _Prime::suffix>::ssig1(s[4]) + ch<_Base>(s[4], s[5], s[6]) + k_vlaue[j] + w[j];
